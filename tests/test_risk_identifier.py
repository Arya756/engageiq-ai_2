...
def test_flags_three_consecutive_low_sessions():
    ri = RiskIdentifier(threshold=0.50, consecutive_sessions=3)

    result = ri.evaluate(student_id="anon_42", session_scores=[0.40, 0.35, 0.30])
...
def test_healthy_student_is_not_flagged():
    ...
    result = ri.evaluate(student_id="anon_10", session_scores=[0.90, 0.88, 0.85, 0.82])
...
def test_one_low_session_mixed_with_good_ones_is_not_consistently_low():
    ...
    # Last 3 scores: 0.40, 0.40, 0.60 -> average 0.467 (< 0.50) but NOT every
    # individual session is below threshold, so this must not be flagged
    # as "consistently low."
    result = ri.evaluate(student_id="anon_9", session_scores=[0.40, 0.40, 0.60])

    assert result.rolling_consecutive_avg == pytest.approx(0.466666, rel=1e-4)
...
def test_only_most_recent_session_low_is_not_consistently_low():
    ri = RiskIdentifier(
        threshold=0.50, consecutive_sessions=3, decline_threshold_pct=90
    )
    ...
    result = ri.evaluate(student_id="anon_11", session_scores=[0.90, 0.88, 0.85, 0.40])
...
def test_fewer_sessions_than_window_cannot_be_flagged_as_consistently_low():
    ...
    result = ri.evaluate(student_id="anon_2", session_scores=[0.10, 0.20])
...
def test_detects_declining_trend_without_timestamps():
    ...
    result = ri.evaluate(
        student_id="anon_44", session_scores=[0.80, 0.75, 0.70, 0.65, 0.60]
    )
...
def test_declining_trend_without_crossing_the_threshold():
    ...
    above the raw threshold -- this is exactly the "0.80 -> 0.70 -> 0.60"
    case from the issue's motivation, and should be flagged even though
    no individual score is anywhere near `threshold`.
    """
    ri = RiskIdentifier(
        threshold=0.20, consecutive_sessions=3, decline_threshold_pct=10
    )

    result = ri.evaluate(
        student_id="anon_45", session_scores=[0.80, 0.75, 0.70, 0.65, 0.60]
    )
...
def test_stable_scores_are_not_flagged_as_declining():
    ...
    result = ri.evaluate(
        student_id="anon_11b", session_scores=[0.70, 0.71, 0.69, 0.70, 0.72]
    )
...
def test_rolling_7day_and_30day_windows_with_timestamps():
    ...
    scores = [0.80, 0.78, 0.82, 0.40, 0.35, 0.30]

    result = ri.evaluate(
        student_id="anon_7", session_scores=scores, session_dates=dates
    )

    assert result.rolling_7day_avg == pytest.approx(0.35)
...
def test_declining_trend_uses_real_week_over_week_with_timestamps():
    ri = RiskIdentifier(
        threshold=0.10, consecutive_sessions=3, decline_threshold_pct=10
    )

    now = datetime(2026, 7, 27, 12, 0, 0)
    dates = [now - timedelta(days=10 - i) for i in range(10)]
    scores = [0.82, 0.80, 0.78, 0.79, 0.81, 0.80, 0.62, 0.60, 0.58, 0.61]
...
def test_students_are_anonymized_by_default():
    ri = RiskIdentifier(threshold=0.50)

    result = ri.evaluate(
        student_id="alex.johnson@school.edu", session_scores=[0.40, 0.35, 0.30]
    )
...
def test_evaluate_rejects_out_of_range_scores_even_without_dates():
    ...
    with pytest.raises(ValueError):
        ri.evaluate(student_id="anon_1", session_scores=[1.2, -0.05, 0.60])
...
def test_rolling_average_returns_none_before_enough_data():
    assert TrendAnalyzer.rolling_average([0.90, 0.85], window=3) is None
...
def test_rolling_average_uses_most_recent_window():
    assert TrendAnalyzer.rolling_average(
        [0.90, 0.40, 0.30, 0.20], window=3
    ) == pytest.approx(0.30)


def test_rolling_average_invalid_window_raises():
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average([0.01, 0.02, 0.03], window=0)
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average([0.01, 0.02, 0.03], window=-1)
...
def test_rolling_average_series_matches_rolling_average_at_each_point():
    ...
    assert series == [None, None, pytest.approx(1.60 / 3), pytest.approx(0.30)]
...
def test_percent_decline_handles_zero_previous():
    assert TrendAnalyzer.percent_decline(0, 0.10) == 0.0


def test_percent_decline_positive_for_a_drop():
    assert TrendAnalyzer.percent_decline(1.0, 0.80) == pytest.approx(20.0)
...
def test_rolling_window_average_exactly_seven_day_boundary():
    ...
    sessions = [
        ScoredSession(score=0.50, timestamp=now - timedelta(days=6)),
        ScoredSession(score=0.90, timestamp=now - timedelta(days=7)),
    ]

    avg = TrendAnalyzer.rolling_window_average(sessions, now, days=7)

    assert avg == pytest.approx(0.50)  # only the day-6 session is included


def test_rolling_window_average_exactly_thirty_day_boundary():
    now = datetime(2026, 7, 27)
    sessions = [
        ScoredSession(score=0.50, timestamp=now - timedelta(days=29)),
        ScoredSession(score=0.90, timestamp=now - timedelta(days=30)),
    ]

    avg = TrendAnalyzer.rolling_window_average(sessions, now, days=30)

    assert avg == pytest.approx(0.50)  # only the day-29 session is included
...
def test_rolling_window_handles_unsorted_input():
    ...
    sessions = [
        ScoredSession(score=0.10, timestamp=now),
        ScoredSession(score=0.90, timestamp=now - timedelta(days=20)),
        ScoredSession(score=0.30, timestamp=now - timedelta(days=1)),
    ]

    window = TrendAnalyzer.rolling_window(sessions, now, days=7)

    assert {s.score for s in window} == {0.10, 0.30}


def test_week_over_week_result_fields_are_named_not_positional():
    now = datetime(2026, 7, 27)
    sessions = [
        ScoredSession(score=0.40, timestamp=now - timedelta(days=1)),
        ScoredSession(score=0.80, timestamp=now - timedelta(days=8)),
    ]

    result = TrendAnalyzer.week_over_week(sessions, now)

    assert result.current_week_avg == pytest.approx(0.40)
    assert result.previous_week_avg == pytest.approx(0.80)


def test_scored_session_validates_score_range():
    with pytest.raises(ValueError):
        ScoredSession(score=1.5, timestamp=datetime(2026, 7, 27))
    with pytest.raises(ValueError):
        ScoredSession(score=-0.05, timestamp=datetime(2026, 7, 27))
...
def test_rolling_average_rejects_out_of_range_scores():
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average([1.2, 0.50, 0.60], window=2)
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average([-0.10, 0.50, 0.60], window=2)


def test_rolling_average_series_rejects_out_of_range_scores():
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average_series([0.50, 2.0, 0.60], window=2)


def test_split_first_second_half_rejects_out_of_range_scores():
    with pytest.raises(ValueError):
        TrendAnalyzer.split_first_second_half([0.50, -0.01, 0.60, 0.70])
...
def test_rolling_average_series_window_larger_than_input():
    assert TrendAnalyzer.rolling_average_series([0.50, 0.60], window=5) == [None, None]
...
def test_rolling_average_series_invalid_window_raises():
    with pytest.raises(ValueError):
        TrendAnalyzer.rolling_average_series([0.10, 0.20, 0.30], window=0)
...
def test_session_exactly_six_days_before_is_included_in_seven_day_window():
    now = datetime(2026, 7, 27)
    sessions = [ScoredSession(score=0.42, timestamp=now - timedelta(days=6))]
...
def test_session_exactly_seven_days_before_is_excluded_from_seven_day_window():
    now = datetime(2026, 7, 27)
    sessions = [ScoredSession(score=0.42, timestamp=now - timedelta(days=7))]
...
def test_previous_week_window_does_not_overlap_current_week_window():
    ...
    current_week_session = ScoredSession(score=0.10, timestamp=now - timedelta(days=2))
    boundary_session = ScoredSession(
        score=0.99, timestamp=now - timedelta(days=6) - timedelta(days=1)
    )  # last day of the *previous* window, first day outside current
    previous_week_session = ScoredSession(
        score=0.90, timestamp=now - timedelta(days=10)
    )
