...
def test_generate_end_to_end():
    ...
    log_curr = EngagementLog(session_id=1, user_id=1, engagement_score=0.8, timestamp=datetime(2026, 6, 8, 9, 10))
    log_prev = EngagementLog(session_id=2, user_id=1, engagement_score=0.9, timestamp=datetime(2026, 6, 1, 9, 10))

    setup_db_mock(mock_db, mock_course, [session_curr], [session_prev], [log_curr], [log_prev])
...
def test_timezone_mismatch_regression():
    ...
    log_aware = EngagementLog(
        session_id=1, user_id=1, engagement_score=0.8, 
...
def test_to_json_safe_dict():
    ...
    session_curr = SessionModel(id=1, title="L1", start_time=datetime(2026, 6, 8, 9, tzinfo=timezone.utc))
    
    score1 = 0.825
    score2 = 0.200
    log_curr1 = EngagementLog(session_id=1, user_id=1, engagement_score=score1, timestamp=datetime(2026, 6, 8, 9, 1, tzinfo=timezone.utc))
    log_curr2 = EngagementLog(session_id=1, user_id=1, engagement_score=score2, timestamp=datetime(2026, 6, 8, 9, 5, tzinfo=timezone.utc))
    
    prev_score = 0.900
    session_prev = SessionModel(id=2, title="L0", start_time=datetime(2026, 6, 1, 9, tzinfo=timezone.utc))
    log_prev = EngagementLog(session_id=2, user_id=1, engagement_score=prev_score, timestamp=datetime(2026, 6, 1, 9, 1, tzinfo=timezone.utc))

    setup_db_mock(mock_db, mock_course, [session_curr], [session_prev], [log_curr1, log_curr2], [log_prev])
    ...
    assert data["at_risk"]["entries"][0]["average_score"] == pytest.approx(expected_risk_avg)
...
def test_render_pdf(monkeypatch):
    ...
    monkeypatch.setattr(gen, "render_html", lambda r: "<html><body>Report</body></html>")
