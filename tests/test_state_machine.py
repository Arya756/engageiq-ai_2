...
def test_initial_state_confirmation(fsm):
    ...
    state = fsm.update(score=85, is_drowsy=False, is_confused=False, timestamp=dt(0))
...
def test_sustained_engaged(fsm):
    ...
    fsm.update(score=90, timestamp=dt(0))
    assert fsm.current_state == EngagementState.ENGAGED
    ...
    fsm.update(score=85, timestamp=dt(2))
    fsm.update(score=95, timestamp=dt(5))
    fsm.update(score=88, timestamp=dt(10))
...
def test_brief_dip_hysteresis(fsm):
    ...
    # Dip into DISTRACTED range (score=30) for 10 seconds (needs 15s to confirm)
    fsm.update(score=30, timestamp=dt(2))
    fsm.update(score=30, timestamp=dt(12))
    ...
    fsm.update(score=85, timestamp=dt(13))
...
def test_sustained_passive(fsm):
    ...
    # Passive range (score=55)
    fsm.update(score=55, timestamp=dt(5))
    assert fsm.current_state == EngagementState.ENGAGED

    # 29 seconds later (total 29s in passive)
    fsm.update(score=55, timestamp=dt(34))
    assert fsm.current_state == EngagementState.ENGAGED

    # 31 seconds elapsed -> should trigger transition
    fsm.update(score=55, timestamp=dt(36))
...
def test_sustained_distracted(fsm):
    ...
    fsm.update(score=30, timestamp=dt(10))
    assert fsm.current_state == EngagementState.ENGAGED

    # 14 seconds in
    fsm.update(score=30, timestamp=dt(24))
    assert fsm.current_state == EngagementState.ENGAGED

    # 16 seconds elapsed -> should trigger transition
    fsm.update(score=30, timestamp=dt(26))
...
def test_gradual_decline(fsm):
    ...
    fsm.update(score=55, timestamp=dt(10))
    fsm.update(score=55, timestamp=dt(45))
    assert fsm.current_state == EngagementState.PASSIVE

    # Drop to DISTRACTED for > 15s
    fsm.update(score=30, timestamp=dt(50))
    fsm.update(score=30, timestamp=dt(68))
    assert fsm.current_state == EngagementState.DISTRACTED
...
def test_drowsy_override_true(fsm):
    ...
    provided the score is also low enough to be plausible (<= 30 by default)."""
    fsm.update(score=90, timestamp=dt(0))

    # Score 20 (normally Distracted, and <= drowsy_max_score), with
    # is_drowsy=True forces DROWSY
    fsm.update(score=20, is_drowsy=True, timestamp=dt(10))

    # 9 seconds elapsed
    fsm.update(score=20, is_drowsy=True, timestamp=dt(19))
    assert fsm.current_state == EngagementState.ENGAGED

    # 11 seconds elapsed
    fsm.update(score=20, is_drowsy=True, timestamp=dt(21))
...
def test_drowsy_override_false(fsm):
    ...
    fsm.update(score=5, is_drowsy=False, timestamp=dt(10))

    # Wait past the Distracted 15s hysteresis
    fsm.update(score=5, is_drowsy=False, timestamp=dt(26))
...
def test_drowsy_flag_gated_by_score(fsm):
    ...
    # score=90 is well above the default drowsy_max_score (30), so the
    # drowsy flag should be ignored and the observation classified as ENGAGED.
    fsm.update(score=90, is_drowsy=True, timestamp=dt(5))
    assert fsm.current_state == EngagementState.ENGAGED
    assert fsm.pending_candidate is None

    # Same principle in the PASSIVE band: score=55 with is_drowsy=True should
    # start (or need) a PASSIVE candidate, not a DROWSY one.
    fsm.update(score=55, is_drowsy=True, timestamp=dt(10))
    fsm.update(score=55, is_drowsy=True, timestamp=dt(41))  # >30s -> confirm PASSIVE
    assert fsm.current_state == EngagementState.PASSIVE
...
def test_drowsy_flag_at_exact_gate_boundary(fsm):
    ...
    fsm.update(score=30, is_drowsy=True, timestamp=dt(10))  # == default gate of 30
    fsm.update(score=30, is_drowsy=True, timestamp=dt(21))  # >10s -> confirm DROWSY
    assert fsm.current_state == EngagementState.DROWSY
...
@pytest.mark.parametrize(
    "score, is_confused", [(90.0, True), (55.0, True), (10.0, True)]
)
def test_confused_override(fsm, score, is_confused):
    ...
    fsm.update(score=85, timestamp=dt(0))
...
def test_confused_overrides_drowsy(fsm):
    ...
    fsm.update(score=20, is_drowsy=True, is_confused=True, timestamp=dt(10))
    fsm.update(score=20, is_drowsy=True, is_confused=True, timestamp=dt(31))
...
def test_timestamp_validation(fsm):
    ...
    fsm.update(score=85, timestamp=dt(10))

    with pytest.raises(ValueError):
        fsm.update(score=85, timestamp=dt(5))


# ---------------------------------------------------------------------------
# 11. Score validation
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("invalid_score", [-1.0, 101.0, math.nan, math.inf, -math.inf])
...
def test_event_system_dedup_behavioral(fsm):
    ...
    fsm.update(score=90, timestamp=dt(0))  # first observation -> one transition
...
def test_listener_exception_does_not_break_update_or_other_listeners(fsm):
    ...
    state = fsm.update(score=90, timestamp=dt(0))
...
def test_history_logging():
    ...
    fsm.update(score=90, timestamp=dt(0))
    fsm.update(score=30, timestamp=dt(5))
    fsm.update(score=30, timestamp=dt(6))
    fsm.update(score=30, timestamp=dt(7))  # This 4th update pushes out the first
...
def test_immediate_engaged_reentry(fsm):
    ...
    fsm.update(score=30, timestamp=dt(10))
    fsm.update(score=30, timestamp=dt(30))
    assert fsm.current_state == EngagementState.DISTRACTED
...
def test_configurable_score_bands():
    ...
    fsm = EngagementStateMachine(passive_min=50.0, engaged_min=80.0)

    # score=75 would be ENGAGED under defaults, but is PASSIVE under these
    # custom thresholds.
    fsm.update(score=90, timestamp=dt(0))
    fsm.update(score=75, timestamp=dt(1))
    fsm.update(score=75, timestamp=dt(32))  # 31s elapsed, > 30s PASSIVE hysteresis
...
def test_configurable_drowsy_max_score():
    ...
    fsm = EngagementStateMachine(drowsy_max_score=60.0)

    fsm.update(score=90, timestamp=dt(0))

    # score=50 would not be drowsy-eligible under the default gate (30), but
    # is eligible under this custom gate (60).
    fsm.update(score=50, is_drowsy=True, timestamp=dt(10))
    fsm.update(score=50, is_drowsy=True, timestamp=dt(21))  # > 10s DROWSY hysteresis
...
def test_configurable_hysteresis_seconds():
    ...
    fsm.update(score=90, timestamp=dt(0))
    fsm.update(score=55, timestamp=dt(1))
    fsm.update(score=55, timestamp=dt(4))  # 3s elapsed < custom 5s -> still ENGAGED
    assert fsm.current_state == EngagementState.ENGAGED

    fsm.update(score=55, timestamp=dt(7))  # 6s elapsed >= custom 5s -> PASSIVE
    assert fsm.current_state == EngagementState.PASSIVE
...
def test_invalid_configuration_raises():
    ...
    with pytest.raises(ValueError):
        EngagementStateMachine(passive_min=80.0, engaged_min=50.0)  # passive > engaged

    with pytest.raises(ValueError):
        EngagementStateMachine(max_history=0)

    with pytest.raises(ValueError):
        EngagementStateMachine(drowsy_max_score=150.0)
