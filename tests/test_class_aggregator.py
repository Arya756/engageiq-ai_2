...
def test_aggregate_normal_class():
    ...
    assert abs(stats.average - 76.5) < 0.1
    assert abs(stats.median - 77.5) < 0.1
...
def test_aggregate_returns_classstats_instance():
    ...
    agg = make_agg()
    assert isinstance(agg.aggregate([70, 80]), ClassStats)


def test_aggregate_all_engaged():
    """When all students score > 70, engaged_pct must be 1.0."""
    agg = make_agg()
    assert agg.aggregate([80, 85, 90, 75, 95]).engaged_pct == 1.0


def test_aggregate_none_engaged():
    """When no students score > 70, engaged_pct must be 0.0."""
    agg = make_agg()
    assert agg.aggregate([60, 55, 40, 65, 30]).engaged_pct == 0.0


def test_aggregate_score_at_threshold_not_engaged():
    """Score exactly at 70 must NOT count as engaged (strictly > 70)."""
    agg = make_agg()
    assert agg.aggregate([70.0]).engaged_pct == 0.0


def test_aggregate_excludes_none_scores():
    """Disconnected students (None) must be excluded from all calculations."""
    agg = make_agg()
    stats = agg.aggregate([80.0, None, 90.0, None, 70.0])
    assert stats.student_count == 3
    assert abs(stats.average - 80.0) < 0.1


def test_aggregate_skips_non_numeric_junk():
    """Non-numeric junk values must be skipped, not raise ValueError."""
    agg = make_agg()
    stats = agg.aggregate([80.0, "N/A", 90.0, "error", {}])
    assert stats.student_count == 2
    assert abs(stats.average - 85.0) < 0.1
...
def test_aggregate_skips_bool_values():
    ...
    """
    agg = make_agg()
    stats = agg.aggregate([80.0, True, 90.0, False])
    # Only 80.0 and 90.0 are valid — bools must be excluded
    assert stats.student_count == 2
    assert abs(stats.average - 85.0) < 0.1
...
def test_update_timeline_stores_average():
    ...
    agg = make_agg()
    agg.update_timeline(10, [80.0, 70.0, 90.0])
    agg.update_timeline(11, [60.0, 70.0, 80.0])
    timeline = agg.get_timeline()
    assert abs(timeline[10] - 80.0) < 0.1
    assert abs(timeline[11] - 70.0) < 0.1


def test_update_timeline_returns_timeline_entry():
    """update_timeline() must return a TimelineEntry instance."""
    agg = make_agg()
    entry = agg.update_timeline(1, [80.0, 90.0])
...
def test_update_timeline_skips_bool_values():
    ...
    agg = make_agg()
    entry = agg.update_timeline(5, [80.0, True, 60.0, False])
    assert entry.student_count == 2
    assert abs(entry.average - 70.0) < 0.1
...
def test_disconnected_minute_does_not_corrupt_session_average():
    ...
    agg = make_agg()
    agg.update_timeline(10, [80.0, 80.0])
    agg.update_timeline(11, [None, None])  # wifi outage
    agg.update_timeline(12, [80.0, 80.0])
    timeline = agg.get_timeline()
...
def test_update_timeline_overwrites_same_minute():
    ...
    agg = make_agg()
    agg.update_timeline(5, [80.0])
    agg.update_timeline(5, [50.0])
    assert abs(agg.get_timeline()[5] - 50.0) < 0.1
...
def test_get_timeline_sorted_by_minute():
    ...
    agg = make_agg()
    agg.update_timeline(3, [80.0])
    agg.update_timeline(1, [70.0])
    agg.update_timeline(2, [75.0])
...
def test_reset_timeline_allows_fresh_start():
    ...
    agg.update_timeline(1, [80.0])
    agg.reset_timeline()
    agg.update_timeline(1, [50.0])
    assert abs(agg.get_timeline()[1] - 50.0) < 0.1
...
def test_detect_dips_stable_class_no_dips():
    ...
    agg = make_agg()
    timeline = {1: 80, 2: 82, 3: 79, 4: 81, 5: 80}
    assert agg.detect_dips(timeline, threshold=0.15) == []
...
def test_detect_dips_raises_on_threshold_above_one():
    ...
    with pytest.raises(ValueError, match="fraction"):
        agg.detect_dips({1: 80, 2: 50}, threshold=15)
...
def test_detect_dips_raises_on_zero_threshold():
    ...
    with pytest.raises(ValueError):
        agg.detect_dips({1: 80}, threshold=0.0)
...
def test_detect_dips_raises_on_negative_threshold():
    ...
    agg = make_agg()
    with pytest.raises(ValueError):
        agg.detect_dips({1: 80}, threshold=-0.1)
...
def test_detect_dips_threshold_boundary_one_allowed():
    ...
    dips = agg.detect_dips({1: 80, 2: 80, 3: 0}, threshold=1.0)
...
def test_detect_dips_returns_engagement_dip_objects():
    ...
    agg = make_agg()
    dips = agg.detect_dips({1: 80, 2: 80, 3: 30}, threshold=0.15)
    for dip in dips:
...
def test_detect_dips_above_average_not_flagged():
    ...
    agg = make_agg()
    dip_minutes = [
        d.minute for d in agg.detect_dips({1: 60, 2: 90, 3: 90}, threshold=0.15)
    ]
...
def test_timeline_feeds_detect_dips_end_to_end():
    ...
    agg = make_agg()
    agg.update_timeline(10, [78.0, 80.0])
    agg.update_timeline(11, [80.0, 82.0])
    agg.update_timeline(12, [None, None])  # outage — must not affect baseline
    agg.update_timeline(14, [40.0, 45.0])  # real dip
    agg.update_timeline(15, [42.0, 44.0])  # still low
